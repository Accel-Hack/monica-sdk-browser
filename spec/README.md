...
ない。v1 API が育つあいだ中身は変わる。**どの契約で作ったかは git の履歴が
記録する**（配信側が `index.json` を出すようになれば、その `revision` も使える）。
...
配信側は索引（`index.json`。全ファイルのパスと sha256、全体の `revision`）を
足す予定で、取り込みはそれを起点にする作りにしてある。索引が配信されるまでは
手元にあるファイルの一覧を取りに行くので、**上流にファイルが増えたことは
検知できない**。取り込みの出力にその旨が出る。
